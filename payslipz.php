...
<body>
  ...
  <form method="post" id="payslips_form" class="sticky-form form-container noprint">
      ...
      <?php 
      if($_SESSION['division'] == 100 || $_SESSION["accessLevel"] == 'ADMIN'){
        $divisionSelectTag = "
      ...
      <div class="form-group center-input">
          <select id="payroll_dropdown" name="payroll_dropdown" class="form-control">
              <option value="1" <?php if($_SESSION['databaseNumber'] == 1) echo 'selected'?>>Executive Staff <?php echo '<h3 class="noprint">'.Payslip::divisions[$division].' Division</h3>'; ?></option>
              <option value="2" <?php if($_SESSION['databaseNumber'] == 2) echo 'selected'?>>Senior Staff <?php echo '<h3 class="noprint">'.Payslip::divisions[$division].' Division</h3>'; ?></option>
              <option value="3" <?php if($_SESSION['databaseNumber'] > 2) echo 'selected'?>>Junior Staff <?php echo '<h3 class="noprint">'.Payslip::divisions[$division].' Division</h3>'; ?></option>
          </select>
      </div>
  ...
  <?php
    ...
    while ($employee = sqlsrv_fetch_array($empsInfo, SQLSRV_FETCH_ASSOC)) {
      //var_dump(Payslip::divisions);
      
  ?>
<div class="page">
  <img src='\assets\img\admarc logo.png' alt='admarc-logo' class='print-logo'>
  <?php echo '<h3 class="screen-only">'.$employee['division'].' </h3>'; ?>
