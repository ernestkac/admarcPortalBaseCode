...
class TaxData {
		...
		if($this->databaseObject){
			...
			if ($results) {
				...
				if ($outputResult) {
					...
					var_dump($row );*/
					$this->fileName = "Tax Returns for ".$this->databaseObject->databaseNames[$this->databaseName]." period ".$PerPost.".csv";
	...
	public	function generateQuery($results) {
		...
		$selectHeader = 'GROUP BY EmpId; ' .
			'GO ' .
			'select Employee.Name ,Employee.empid , '.
			'CONVERT(VARCHAR(10),BirthDate,104)  birthday,W2EmpName.NameSuffix as Gender, ' .
                'XHR_Job.JobTitle as designation';
