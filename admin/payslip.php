...
<?php 
    ...
    if (!isset($_SESSION['csrf_token']) || !isset($_SESSION["name"])) {
        header("Location: index");
    }

    // Basic POST CSRF validation: reject or ignore POSTs with invalid token
    if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        $csrf_post = $_POST['csrf_token'] ?? '';
        if (!hash_equals($_SESSION['csrf_token'] ?? '', (string)$csrf_post)) {
            // If this is an AJAX action, return an error immediately
            if (
              isset($_POST["action"]) ||
              ...
              isset($_POST["q"])
                ...
                echo 'Invalid CSRF token'.$_SESSION['csrf_token'].'-'.$csrf_post;
                exit;
            }
            // For normal form submissions set a visible message and clear POST so later handlers don't act
            $empidInputValue = 'Invalid form submission (CSRF)';
            $_POST = [];
        }
    }
    ...
    if (isset($_POST['period_checkbox']) && $_POST['period_checkbox'] == 'on' ) {
      ...
      if($payslipObject->culPerPost == $payslipObject->aprovedPerPost){
        ...
        $OperationLog = 'ADMIN restricting view of period '.$payslipObject->culPerPost;
      }
        
      else{
        $payslipObject->aprovePerPost();
        $OperationLog = 'ADMIN allowing view of period '.$payslipObject->culPerPost;
      }  
            ...
            if ($row != null) {
              ...
              if($_SESSION['division'] == $row['division'] || $_SESSION['division'] == 100 || $_SESSION["accessLevel"] == 'ADMIN'){
                ...
                $databaseNumber = $dbNumber;
                  ...
                  $_SESSION['profile']["name"] = trim($row['name']);
    ...
    elseif (isset($_POST['monthLeft']) ) {
      ...
      $dateFormat = 'Ym';
      $date = DateTime::createFromFormat($dateFormat,$_SESSION["perpost"]);
      $date->modify('-1 month');
      $date->modify('-1 month');
  ...
  $print_name_title = $_SESSION['profile']['name'];
    ...
    if ($OperationLog == '') {
      $OperationLog = $EmployeementNumber ." ADMIN payslip view month : ".$_SESSION['fromMonth']." to ".$_SESSION['toMonth'];
    }
...
<html>
  <head>
    ...
    <title>Admarc payslip portal</title>
    <!-- expose CSRF token to JS safely -->
    <meta name="csrf-token" content="<?php echo htmlspecialchars($_SESSION['csrf_token'] ?? '', ENT_QUOTES | ENT_SUBSTITUTE); ?>">
    ...
      <!-- Font Awesome CDN -->
    <link
      rel="stylesheet"
  ...
  <body>
    ...
    <div class="space-div noprint screen-only">
    ...
    <script src="assets\js\searchv2.js"></script>
    ...
    <script type="text/javascript">
      ...
      function openchart() {
        ...
        fetch("payslip.php", {
            ...
            body: "action=modal_click&csrf_token=<?php echo urlencode($_SESSION['csrf_token']); ?>"
        })
